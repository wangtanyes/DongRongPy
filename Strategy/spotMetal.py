...
class plan():
    ...
    def draw(self,x,y):
        plt.rcParams['axes.unicode_minus'] = False      #   可以解决负号显示问题
        plt.rcParams['font.sans-serif'] = ['SimHei']    #  解决中为显示问题
        plt.rcParams['font.size'] = 10
        # plt.xlim("最小值","最大值")       # 控制x轴的范围
        # plt.ylim("最小值","最大值")       # 控制y轴的范围
        # plt.xticks(range(2))
        dates = []
        ...
        # ax1 = fig.add_subplot(1,1,1)        # 添加子图的形式进行画图
        ax1.xaxis.set_major_formatter(DateFormatter('%Y-%m-%d'))        #  设置x轴的显示格式,%Y-%m-%d以年月日显示
        plt.plot(dates, y, 'b-', lw=1)

        # plt.xticks(np.arange(0,1,step=0.2), ('Tom', 'Dick', 'Harry', 'Sally', 'Sue'),rotation=45)      #  设置x轴的各点之间的间距，以及x轴各点的值

        plt.xticks(pd.date_range(start='2007-01-08',end='2018-04-14',freq='Y'),rotation=45)     # 设置y轴的长度
    ...
    def listValue(self, time, firstValue, kind, length):
        ...
        for i in range(len(time)-length+1):
            ...
            if "铜" in kind or "铝" in kind:
                val = (value)*0.35/(length*firstValue)
                values.append(val)
            else:
